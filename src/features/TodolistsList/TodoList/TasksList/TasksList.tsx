...
export const TasksList: FC<TasksListPropsType> = memo(({ todolistId, filter, entityStatus }) => {
	...
	const getFilteredTasks = (allTasks: TaskDomainType[], filterValue: FilterValuesType) => {
		switch (filterValue) {
			case "active":
	...
	const tasksItems: JSX.Element = tasks.length
		? <ul ref={listRef} className={s.tasksWrapper}>
			{tasksForTodoList.map((task) => {
				return (
					<li key={task.id}>
						<Task entityTaskStatus={task.entityStatus} entityStatus={entityStatus} todolistId={todolistId} task={task} />
					</li>
				)
			})}
		</ul>
